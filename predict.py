...
def predict(model, directory, num_workers, max_count=sys.maxsize):
    ...
    result_file_path = path.join(directory, 'prediction.txt')
    with open(result_file_path, 'w') as f:
        for minibatch in tqdm(behaviors_dataloader, desc="Generating predictions"):
            impression_id = minibatch['impression_id'][0]
...
if __name__ == '__main__':
    ...
    checkpoint_path = latest_checkpoint(path.join('./checkpoint/checkpoint/NAML/fina_model.pth'))
    ...
    model.eval()
    predict(model, './data/test',config.num_workers)
